...
class KnowledgeBaseSearchTask(AbstractServiceNowTask):
    ...
    def cheat(self, page: Page, chat_messages: list[str]) -> None:
        ...
        custom_print(page, search, "fill", value=f'"{self.item}"')
        search.fill(f'"{self.item}"')
        post_event(page, "fill", locator=search, value=f'"{self.item}"')
        ...
        with page.expect_navigation():
            if self.search_by_title:
                article_locator = iframe.locator(f'a.kb-title:has-text("{self.kb_article_title}")')
                custom_print(page, article_locator, "click")
                article_locator.click()
                post_event(page, "click", locator=article_locator)
            else:
                article_locator = iframe.locator("a.kb-title").first
                custom_print(page, article_locator, "click")
                article_locator.click()
                post_event(page, "click", locator=article_locator)
...
class AddCommentToKnowledgeArticleTask(AbstractServiceNowTask, CompositionalBuildingBlockTask):
    ...
    def cheat(self, page: Page, chat_messages: list[str]) -> None:
        ...
        comment_button = frame.locator("button.comment-text")
        custom_print(page, comment_button, "click")
        comment_button.click()
        post_event(page, "click", locator=comment_button)
        
        rich_text_area = frame.frame_locator('iframe[title="Rich Text Area"]').locator("html")
        custom_print(page, rich_text_area, "click")
        rich_text_area.click()
        post_event(page, "click", locator=rich_text_area)
        
        comments_field = frame.frame_locator('iframe[title="Rich Text Area"]').get_by_label("Comments")
        custom_print(page, comments_field, "fill", value=self.comment)
        comments_field.fill(self.comment)
        post_event(page, "fill", locator=comments_field, value=self.comment)
        
        submit_button = frame.get_by_role("button", name="Submit")
        custom_print(page, submit_button, "click")
        submit_button.click()
        post_event(page, "click", locator=submit_button)
