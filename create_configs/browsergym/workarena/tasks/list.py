...
from .utils.action_capture import inject_bgym_actions, custom_print, post_event
...
class SortListTask(ServiceNowListTask):
    ...
    def cheat(self, page: Page, chat_messages: list[str]) -> None:
        ...
        ## Custom print to log the inner html of the list_filter_toggle
        custom_print(page,iframe.locator(".list_filter_toggle"),"click")

        iframe.locator(".list_filter_toggle").click()
        post_event(page, "click", locator=iframe.locator(".list_filter_toggle"))
        ...
        for i, (field_txt, dir_txt) in enumerate(zip(sort_fields_txt, sort_dirs_txt)):
            ...
            custom_print(page,filter.get_by_role("button", name="Add Sort"),"click")
            filter.get_by_role("button", name="Add Sort").click()

            # TODO: Hack to solve bug where the sort condition has not yet appeared
            page.wait_for_timeout(500)
            post_event(page, "click", locator=filter.get_by_role("button", name="Add Sort"))
            ...
            custom_print(page,field_selector,"select_option", value=field_txt)
            field_selector.select_option(field_txt)
            post_event(page, "select_option", locator=field_selector, value=field_txt)
            # Choose sort order
            logging.debug(f"Choosing sorting direction {dir_txt}")
            custom_print(page,dir_selector,"select_option", value=dir_txt)
            dir_selector.select_option(dir_txt)
            post_event(page, "select_option", locator=dir_selector, value=dir_txt)
        ...
        with page.expect_event("framenavigated", predicate=n_events_passed):
            #get_full_xpath
            custom_print(page,filter.get_by_label("Run filter"),"click")
            filter.get_by_label("Run filter").click()
            post_event(page, "click", locator=filter.get_by_label("Run filter"))
    def validate(
...
class FilterListTask(ServiceNowListTask):
    ...
    def cheat(self, page: Page, chat_messages: list[str]) -> None:
        ...
        while iframe.locator(".filerTableAction.deleteButton:visible").count() > 0:
            ...
            custom_print(page,iframe.locator(".filerTableAction.deleteButton:visible").nth(0),"click")
            iframe.locator(".filerTableAction.deleteButton:visible").nth(0).click()
            post_event(page, "click", locator=iframe.locator(".filerTableAction.deleteButton:visible").nth(0))
        ...
        for i in range(len(self.filter_columns)):
            ...
            if i > 0:
                ...
                custom_print(page,iframe.locator(
                ...
                post_event(page, "click", locator=iframe.locator(
                    f'.filterToolbar .filerTableAction:text-is("{self.filter_kind}")'
                ))
            ...
            custom_print(page,field_selector,"select_option", value=self.filter_columns[i])
            field_selector.select_option(self.filter_columns[i])
            post_event(page, "select_option", locator=field_selector, value=self.filter_columns[i])
            ...
            custom_print(page,row.locator("select.condOperator"),"select_option", value=operator_symbol)
            row.locator("select.condOperator").select_option(operator_symbol)
            post_event(page, "select_option", locator=row.locator("select.condOperator"), value=operator_symbol)
            # Fill in the value
            ...
            if type_ in ["string", "reference", "translated_text"]:
                ...
                custom_print(page,input_field,"fill", value=self.filter_values[i])
                fill_text(
                    page=page,
                    iframe=iframe,
                ...
                post_event(page, "fill", locator=input_field, value=self.filter_values[i])
            else:
                # expect a selector
                logging.debug("filling in selector")
                # Find the value input field
                input_field = row.locator("#value select")
                custom_print(page,input_field,"select_option", value=self.filter_values[i])
                input_field.select_option(self.filter_values[i])
                post_event(page, "select_option", locator=input_field, value=self.filter_values[i])
        custom_print(page,iframe.locator(".filterToolbar").get_by_text("Run"),"click")
        iframe.locator(".filterToolbar").get_by_text("Run").click()
        post_event(page, "click", locator=iframe.locator(".filterToolbar").get_by_text("Run"))
    def validate(
        self, page: playwright.sync_api.Page, chat_messages: list[str]
    ) -> Tuple[float, bool, str, dict]:
        """
