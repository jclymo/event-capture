...
let captureStream = null;
  ...
  function recordEvent(event) {
    console.log("recordEvent", {isRecording, mediaRecorder, chunks});
    if (!isRecording || (mediaRecorder == null) || (!chunks || chunks.length === 0)) {
      console.log("trying to not record event");
      return;
    }
    ...
    const eventData = {
      ...
      timestamp: Date.now(),
      url: window.location.href,
      // screenCapture: take_screenshot(null),
      // previousScreenCapture: screenshotFromChunksAtOffset(null,1),
      target: {
        tag: event.target.tagName,
        id: event.target.id,
        class: event.target.className,
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === "startRecording") {
      ...
      console.log("startRecording", message.taskId);
      startScreenCapture(message.body.currentTab.id);
      sendResponse({status: "recording started"});
    } else if (message.action === "stopRecording") {
      stopRecording();
      mediaRecorder && mediaRecorder.stop();  //stopScreenCapture();
...
// The screen capture parts

function startScreenCapture(currentTabId) {
    ...
    async (streamId) => {
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          // downloadLink.click();
          window.close(); // maybe need to comment this out
        };
